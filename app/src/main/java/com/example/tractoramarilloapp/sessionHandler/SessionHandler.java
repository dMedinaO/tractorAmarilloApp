...
public class SessionHandler {
    ...
    public int createSession(String TagNFC){
        ...
        //preguntamos si existen sesione activas
        if (activeSession.size() == 0){//no hay sesiones activas

            //preguntamos si la pulsera es del tipo jefe o del tipo operador
        ...
        }else{//existen sesiones activas, pueden ser una o varias
            ...
            if (activeSession.size() == 1){//sesion unica
                if (activeSession.get(0).getSessionKind().equals("2")){//sesion de operario, no se puede trabajar porque el operario esta trabajando y es la unica sesion activa
                    response = -2;//sesion activa para operador y en estado unico
                    ...
                    if (kindTag == 1){//jefe desea inicar sesion... NO SE PUEDE
                        ...
                        response = -3;//error de jefe iniciando sesion activa en jefe
                ...
                if (this.isBossActive(activeSession)){
                    ...
                    if (kindTag == 1){
                        response = -3;//error asociado a conflictos de jefes
                        ...
                        if (value == -1){
                            response = -1;
                            Log.e(TAG, "SESSION ACTIVE IS UNIQUE AND BOSS AVAILABLE ERROR INSERT OPERATOR");
                        }else{
                            Log.e(TAG, "SESSION ACTIVE IS UNIQUE AND BOSS AVAILABLE OK INSERT OPERATOR");
                            response=0;
                        }
                    }
                }else{
                    Log.e(TAG, "SESSION ACTIVE OPERATOR AVAILABLE");
                    response = -1;//no se puede iniciar sesion
                }
            }
        }
